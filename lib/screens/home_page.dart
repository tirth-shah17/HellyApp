...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> generatePdfs() async {
    ...
    try {
      final employeeList =
          await ApiService.uploadExcelAndGetPdfLinks(selectedFilePath!);
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          mainAxisAlignment:
              MainAxisAlignment.start, // Changed from center to top alignment
                    ...
                    return EmployeePdfCard(
                      ...
                      onDownload: () async {
                        if (employee.pdfUrl != null) {
                          try {
                            final fullUrl =
                                'https://backend-helly.onrender.com${employee.pdfUrl}';
                            final response = await http.get(Uri.parse(fullUrl));

                            if (response.statusCode == 200) {
                              final dir = await getTemporaryDirectory();
                              final filePath =
                                  '${dir.path}/${employee.name}_${employee.id}.pdf';
                              final file = File(filePath);
                              await file.writeAsBytes(response.bodyBytes);

                              await Share.shareXFiles([XFile(filePath)],
                                  text: 'Payslip for ${employee.name}');
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                      'Shared payslip of ${employee.name}!'),
                                  backgroundColor: Colors.green,
                                ),
                              );
                            } else {
                          ...
                          try {
                            ...
                            if (response.statusCode == 200) {
                              ...
                              Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) =>
                                      PdfPreviewPage(filePath: localPath),
                                ),
                              );
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(
